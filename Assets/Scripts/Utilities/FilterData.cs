...
public class FilterData : MonoBehaviour
{
    ...
    public float input2;
    public float input3;
    public float input_filtered1;
    public float input_filtered2;
    public float input_filtered3;

    void Start()
    {
        Low1 = new IIRFilter(60, 5); // 2nd order low pass butterworth 5Hz at 60Hz sampling rate
        Low2 = new IIRFilter(60, 5); // 2nd order low pass butterworth 5Hz at 60Hz sampling rate
        Low3 = new IIRFilter(60, 5); // 2nd order low pass butterworth 5Hz at 60Hz sampling rate
        OneShotLow = new IIRFilter(60, 5); 
    ...
    void FixedUpdate()
    {
        ...
        input_filtered1 = filter(input1, Low1);
        input_filtered2 = filter(input2, Low2);
        input_filtered3 = filter(input3, Low3);
    }
    public List<float> OneShotLowPassFilter(List<float> dataStream)
    {
        List<float> filtered = new List<float>();
        foreach(float data in dataStream)
        {
            filtered.Add(filter(data, OneShotLow));       
        }
        return filtered;
    }
