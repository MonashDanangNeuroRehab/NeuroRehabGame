...
public class ErrorQuantifier : PostProcessProvider
{
    // General variables
    ...
    public GameObject OptimizedZone;
    public GameObject LeftOutlineHand;
    public GameObject RightOutlineHand;
    ...
    private string _measuringNotif = "Measuring...";
    ...
    public void Start()
    {
        ...
        _filterObject = GameObject.Find("FilterData");
        _filter = _filterObject.GetComponent<FilterData>();
        _sphere = GameObject.Find("Sphere");
        _child = _sphere.transform.GetChild(0).gameObject;
    ...
    private void processHand(Hand hand)
    {
        if (hand != null)
        {            
            _timeVector.Add(Time.realtimeSinceStartup - _initialTime);
            _palmPositionDisplacement.Add(Vector3.Distance(hand.GetPalmPose().position, _initialPalmPosition));
            _wristPositionDisplacement.Add(Vector3.Distance(hand.WristPosition.ToVector3(), _initialWristPosition));
            _palmarAngles.Add(Vector3.Angle(hand.PalmarAxis(), _initialPalmarAxis));
            _palmarAngularVelocity.Add(Vector3.Angle(hand.PalmarAxis(), _prevPalmarAxis) / Time.fixedDeltaTime);
            _distalAngles.Add(Vector3.Angle(hand.DistalAxis(), _initialDistalAxis));
            _filter.input1 = _distalAngles[_distalAngles.Count - 1];
            _filteredDistalAngles.Add(_filter.input_filtered1);
            _distalAngularVelocity.Add(Vector3.Angle(hand.DistalAxis(), _prevDistalAxis) / Time.fixedDeltaTime);
            _prevPalmarAxis = hand.PalmarAxis();
            _prevDistalAxis = hand.DistalAxis();

            // Debug.Log(Vector3.Dot(Vector3.Cross(hand.DistalAxis(), _initialDistalAxis), Camera.main.transform.TransformDirection(Vector3.up)));
            // Location
            _child.transform.position = hand.GetPalmPose().position;
            _locations.Add(_child.transform.localPosition);
            // OptimizedZone.transform.position = hand.GetPalmPose().position;
            // OptimizedZone.transform.rotation = hand.GetPalmPose().rotation;
            // Debug.Log(inputLeapProvider.transform.position);
        }
    }
    private void calibrateHand(Hand hand)
    {
        if (hand != null)
        {
            // Debug.Log(LeftOutlineHand.GetComponent<SkinnedMeshRenderer>().material.color);
            if (!_isHandFirstDetected)
            {
                _prevPalmarAxis = hand.PalmarAxis();
                _prevTime = Time.realtimeSinceStartup;
                OptimizedZone.transform.position = inputLeapProvider.transform.TransformPoint(idealLocation);
            ...
            else if (!_isCalibrated)
            {
                ...
                if (_deltaTime > _calibrationTime)
                {
                    ...
                    if (_calibrationCounter >= 3)
                    {
                        ...
                        _sphere.transform.position = _initialPalmPosition;
                        Vector3 test = inputLeapProvider.transform.InverseTransformPoint(hand.GetPalmPose().position);
                        Debug.Log(test.x);
                        Debug.Log(test.y);
                        Debug.Log(test.z);
                    }
