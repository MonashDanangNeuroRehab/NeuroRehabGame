...
public class RehabMiniGame1 : PostProcessProvider
{
    ...
    private string _palmDownNotif = "Please flip your palm down";
    ...
    private float _angleLowerLimit = 10;
    private float _angleUpperLimit = 145;
    ...
    public void Start()
    {
        ...
        if (!standAloneMode)
        {
            ...
            gameNameText.text = "Hand Flipping";
        }
    }
            ...
            else
            {
                if (!_isStatusUpdated)
                {
                    if (!_isCalibrationDone)
                    {
                        ...
                        if (HandID == LEFT_HAND)
                        {
                            calibrationZone.transform.position = new Vector3(0.02303664f, 1.090155f, 0.1301741f);
                            // calibrationZone.transform.parent = Camera.main.transform;
                            // calibrationZone.transform.localPosition = new Vector3(0.08521278f, -0.1869333f, 0.8380312f);
                        }
                    ...
                    else
                    {
                        ...
                        if (_handFlipsCounter < HAND_ACTION_MAX)
                        {
                            if (_isPalmUp)
                            {
                                if (_gameStarted)
                                ...
                                playbackProvider.ChooseRecording(HandID + "PalmDown");
    ...
    private void SaveToFileDebug()
    {
        ...
        File.WriteAllLines(Application.persistentDataPath + "/game1_debug_baseline.txt", baseline);
        File.WriteAllLines(Application.persistentDataPath + "/game1_debug_player.txt", player);
        File.WriteAllLines(Application.persistentDataPath + "/game1_debug_playerSegmented.txt", playerSegmented);
