...
// Wrist Flexion
public class RehabMiniGame5 : PostProcessProvider
{
    ...
    private string _palmUpNotif = "Please flex your wrist upward";
        ...
        while (true)
        {
            ...
            else
            {
                if (!_isStatusUpdated)
                {
                    ...
                    {
                        if (_handFlipsCounter < HANDFLIPMAX)
                        {
                            if (_isPalmUp)
                            {
                                if (_gameStarted)
                                ...
                                playbackProvider.ChooseRecording(HandID + "WristFlexionDown");
                            ...
                            else
                            {
                                ...
                                playbackProvider.ChooseRecording(HandID + "WristFlexionUp");
                                scoreText.text = Mathf.Round(calculatedScore).ToString();
                                if (!standAloneMode)
                                {
                                    gameManager.totalScore += calculatedScore;
                        ...
                        else
                        {
                            ...
                            statusBarControl.setIndicatorLocation(0);
                            isGameCleared = true;
                            if (!standAloneMode)
                            {
    ...
    private void SaveToFileDebug()
    {
        ...
        File.WriteAllLines(Application.persistentDataPath + "/game5_debug_baseline.txt", baseline);
        File.WriteAllLines(Application.persistentDataPath + "/game5_debug_player.txt", player);
        File.WriteAllLines(Application.persistentDataPath + "/game5_debug_playerSegmented.txt", playerSegmented);
